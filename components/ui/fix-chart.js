...
function ChartTooltipContent({ active, payload, className, indicator = 'dot', hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey }: { active?: boolean; payload?: any[]; label?: string; labelFormatter?: (label: any, payload: any[]) => React.ReactNode; labelClassName?: string; formatter?: (value: any, name: any, item: any, index: number, payload: any) => React.ReactNode; color?: string; nameKey?: string; labelKey?: string; hideLabel?: boolean; hideIndicator?: boolean; indicator?: 'line' | 'dot' | 'dashed'; className?: string }) {
...
function ChartLegendContent({ className, hideIcon = false, payload, verticalAlign = 'bottom', nameKey }: React.ComponentProps<'div'> & { payload?: Array<{ value: string; dataKey?: string | number; color?: string }>; verticalAlign?: 'top' | 'middle' | 'bottom'; hideIcon?: boolean; nameKey?: string }) {
  const { config } = useChart()
...
`;
fs.writeFileSync('components/ui/chart.tsx', code);
console.log('chart.tsx written successfully');